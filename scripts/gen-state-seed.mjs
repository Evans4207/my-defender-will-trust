...
// ---------------------------------------------------------------------------
// Ancillary instruments (POA, healthcare directive).
//
// Deliberately near-empty. docs/ANCILLARY_RULES_GAPS.md explains why: of the
// four states docs/STATE_COMPLIANCE_DOSSIER.md §4 has researched, three do not
// fit what the rules layer can express, and forcing them in would mean choosing
// a legal reading in a seed file. Florida is the exception and is seeded here as
// the pilot, per that memo's recommendation — one real state end to end before
// forty-six more.
//
// Every state absent from this map keeps failing closed for that instrument.
// That is the point of per-state fail-closed: this map grows one counsel-approved
// jurisdiction at a time, and no code changes when it does.
//
// [instrument, rule_key, value, citation]
const ANCILLARY = {
  FL: [
    [
      ...
      "witnesses_required",
      { count: 2 },
      "Fla. Stat. §709.2105 (signed by the principal and by two subscribing witnesses)",
    ],
    [
      "poa",
      "notarization_required_for_document",
      { required: true },
      "Fla. Stat. §709.2105 (acknowledged before a notary public or as otherwise provided in s. 695.03)",
    ],
  ],
};

for (const code of JURISDICTIONS) {
  const rows = [...ruleRows(code), ...(ANCILLARY[code] ?? [])];
